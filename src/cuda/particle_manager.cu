...
__global__ void __launch_bounds__(128, 8) moveParticles(Particle * particles, Config conf, Scene scene, int64_t seed) {
	...
	if (in_x < conf.numParticles) {
        ...
        velD.z += simplexNoise(make_float3(newPosD.x, newPosD.y, -newPosD.z), 1, seed) * conf.particleVelocityLowFreqNoiseFactor.z;
        velD.y += simplexNoise(make_float3(newPosD.x, -newPosD.y, newPosD.z), 1, seed) * conf.particleVelocityLowFreqNoiseFactor.y;
        velD.x += simplexNoise(make_float3(-newPosD.x, newPosD.y, newPosD.z), 1, seed) * conf.particleVelocityLowFreqNoiseFactor.x;

        velD.z += simplexNoise(make_float3(newPosD.x, newPosD.y, -newPosD.z), 100, seed) * conf.particleVelocityHighFreqNoiseFactor.z;
        velD.y += simplexNoise(make_float3(newPosD.x, -newPosD.y, newPosD.z), 100, seed) * conf.particleVelocityHighFreqNoiseFactor.y;
        velD.x += simplexNoise(make_float3(-newPosD.x, newPosD.y, newPosD.z), 100, seed) * conf.particleVelocityHighFreqNoiseFactor.x;
